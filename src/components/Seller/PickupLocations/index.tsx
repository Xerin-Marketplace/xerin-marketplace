...
import { CheckCircle2, MapPin, Plus, RefreshCw, Search, Star, Trash2, X } from "lucide-react";
...
export default function SellerPickupLocations() {
  ...
  const choose = async (placeId: string) => { setBusy(true); try { const next = await usersApi.getMapPlace(placeId, "TZ"); setLocation(next); setSearch(next.formatted_address); setSuggestions([]); } catch (e) { toast.error(message(e)); } finally { setBusy(false); } };
  const close = () => { setOpen(false); setForm(empty); setSearch(""); setLocation(null); setSuggestions([]); };
  const create = async () => {
    if (!location || !form.contact.trim() || !form.phone.trim()) { toast.error("Select the exact map location and enter the pickup contact and phone"); return; }
    setBusy(true);
    try {
      await sellerPickupApi.create({ label: form.label.trim(), formatted_address: location.formatted_address, country: location.country || "Tanzania", region: location.region || location.city || "Unknown", city: location.city || location.region || "Unknown", district: location.district, ward: location.ward, street: location.street, postal_code: location.postal_code, place_id: location.place_id, latitude: Number(location.latitude), longitude: Number(location.longitude), pickup_contact_name: form.contact.trim(), pickup_phone: form.phone.trim(), pickup_instructions: form.instructions.trim() || null, is_default: rows.length === 0, is_active: true });
  ...
  const remove = async (id: string) => { if (!window.confirm("Remove this pickup location?")) return; setBusy(true); try { await sellerPickupApi.remove(id); toast.success("Pickup location removed"); await load(); } catch (e) { toast.error(message(e)); } finally { setBusy(false); } };
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    </div><div className="flex flex-col-reverse gap-2 border-t p-4 dark:border-white/10 sm:flex-row sm:justify-end"><button onClick={close} className="min-h-11 rounded-xl border px-5 text-sm font-semibold">Cancel</button><button disabled={busy || !location} onClick={() => void create()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-orange px-5 text-sm font-bold text-white disabled:opacity-50">{busy && <RefreshCw className="animate-spin" size={15}/>}Save pickup location</button></div></div></div>}
  </div>;
}
