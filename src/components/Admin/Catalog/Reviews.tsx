...
<div className="admin-catalog-toolbar flex-col sm:flex-row sm:items-center sm:justify-between">
<h3 className="font-semibold">Customer reviews</h3>
<button onClick={() => void load()} className="rounded-xl border px-4 py-2 text-sm font-semibold">Refresh</button>
</div>{loading ? <p className="p-10 text-center text-gray-500">Loading reviews...</p> : error ? <p className="p-10 text-center text-red-600">{error}</p> : rows.length === 0 ? <p className="p-10 text-center text-gray-500">No reviews found.</p> : <div className="overflow-x-auto">
<table className="w-full min-w-[700px] text-left text-sm">
<thead className="bg-gray-50">
<tr>{["Customer","Product","Rating","Review","Status","Date"].map(x => <th key={x} className="px-4 py-3">{x}</th>)}</tr>
</thead>
<tbody className="divide-y">{rows.map(row => <tr key={row.id}>
<td className="px-4 py-3">{row.user_id?.slice(0,8) ?? "—"}</td>
<td className="px-4 py-3">{row.product_id?.slice(0,8) ?? "—"}</td>
