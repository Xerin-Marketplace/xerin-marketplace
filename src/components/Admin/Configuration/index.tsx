...
import {
  ...
  listLogisticsZones,
  releaseEscrowHold,
  resendLogisticsAdministratorCredentials,
    ...
    <>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <Card title="Xerin Express Domestic Standards" icon={Truck}>
          ...
          <button type="button" disabled={savingStandard} onClick={()=>void addStandard()} className="rounded-xl bg-black px-4 py-2.5 text-sm font-semibold text-black">{savingStandard?"Saving…":"Add service standard"}</button>
...
function LogisticsCompanies() {
  ...
  const [form, setForm] = useState<LogisticsCompanyForm>({
    ...
    admin_phone: "", scope: "local", status: "pending",
  ...
  const create = async () => {
    ...
    if (!form.name.trim() || !form.code.trim() || !form.admin_first_name.trim() ||
        !form.admin_last_name.trim() || !form.admin_email.trim()) {
      toast.error("Complete the company and administrator fields.");
      return;
    }
    setCreating(true);
    try {
    ...
    <section className="grid gap-5 xl:grid-cols-[390px_1fr]">
      <Card title="Register Logistics Company" icon={Building2}>
        <div className="mb-4 rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-blue-800">
          This creates the company and its primary administrator. Xerin securely generates the temporary password and emails the login credentials directly to the administrator.
        </div>
        ...
        <p className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-800">The admin never creates or sees the password. A secure temporary password is generated by the backend and sent only to the login email.</p>
            ...
            <table className="w-full min-w-[900px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.map((row) => <tr key={row.id}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3"><button className="text-xs font-semibold text-blue" onClick={async () => {
                    ...
                    }}>Resend credentials</button><button className="text-xs font-semibold text-red-600" onClick={async () => {
